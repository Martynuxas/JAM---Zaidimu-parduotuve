...
class vertinimuRed extends Component {
	constructor(props)
	{
		super(props)
		this.state = {
	...
	submitas = event =>{
		...
		let Comment = this.state.description

		const duomenys = {
			Comment
        };
        axios.patch('http://localhost:8000/rate/'+ScoreValue, duomenys, {withCredentials: "true"} ) 
       .then(response =>{
		   alert("Komentaras redaguotas!");
		   return window.location.href = "/vertinimai";
       })
       .catch(error => {
		   alert("Komentaras neredagotas!");
...
<div class="banner">
		...
		<div class="d-flex justify-content-center h-100">
                ...
                <form>
                    <h3 class="text-virsuj">Komentaro redagavimas</h3>
                
                        <h3 class="text-virsuj">Komentaras</h3>
                        <div class="input-group mb-3">
							<div class="input-group-append">
								<span class="input-group-text"><i class="fas fa-edit"></i></span>
                                <textarea type="aprasymas" placeholder="Komentaras" cols="25" rows="8" value={this.state.description} onChange={this.handleDescriptionChange}></textarea>
							</div>
						</div>
							<div class="d-flex justify-content-center mt-3 login_container">
							<button onClick={this.submitas} type="submit" name="button" class="btn login_btn">Redaguoti</button>
				   </div>
					</form>
				</div>           
            </div>
